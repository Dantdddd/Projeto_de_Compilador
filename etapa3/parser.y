...
%type <arvore> elemento_lista declaracao_variavel_global definicao_funcao bloco_comandos
%type <arvore> lista_comandos elementos_lista_comandos literal retorno_funcao expressao
%type <arvore> lista_argumentos_opcional tipo lista_parametros_opcional lista_parametros
...
programa:
      lista ';'
      {
          /* $1 é a lista (pode ser NULL). Tornamos $1 a raiz do parser. */
          arvore = $1;
          $$ = $1;
      }
    | %empty
...
declaracao_variavel_global: TK_VAR TK_ID TK_ATRIB tipo {
    ...
    asd_add_child($$, buffer);  // nome da variável
    asd_add_child($$, $4);  // tipo (inteiro, decimal, etc)
